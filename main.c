...
enum STATE { TERM, READY, RUN, NEW, WAIT };
...
typedef struct tcb {
    ...
    int id;
    enum STATE state;
    int waiting_for;  // ID of the thread the current thread is waiting for
} TCB;
...
int create(void (*start_routine)(void)) {
    ...
    threads[id].id = id;
    threads[id].waiting_for = -1;  // Initialize waiting_for to -1 for all threads
...
void join(int tid) {
    if (tid >= 0 && tid < MAX_THREADS && threads[tid].state != TERM) {
        threads[cur].state = WAIT;
        threads[cur].waiting_for = tid;
        dispatch(SIGALRM);
    }
}
...
int main() {
    ...
    dispatch(SIGALRM);
    run(tid);
    join(tid2); // Main thread will wait for tid2 to finish
    run(tid2);
